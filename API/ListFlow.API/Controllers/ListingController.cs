...
public class ListingController : ControllerBase
{
    ...
    public ActionResult<Listing> GetCrossPost(string itemNumber)
    {
        ...
        if (!listing.Success) return NotFound(listing.ErrorMessage);
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Listing updatedlisting)
    {
        var result = await _listingService.Update(updatedlisting).ConfigureAwait(false);
    ...
    public async Task<IActionResult> UpdateDescription(string itemNumber, [FromBody] DescriptionDto description)
    {
        var result = await _listingService.UpdateDescription(itemNumber, description.Description);
    ...
    {
        var listing =  _listingService.FindListingsByItemNumber(itemNumber);
    ...
    [HttpGet("{saleChannelName}/toCrossPost")]
    public ActionResult<IEnumerable<CrossListingResult>> GetListingsToCrossPost(string saleChannelName)
    {
        var listing = _listingService.GetListingsToCrossPost(saleChannelName);
        return Ok(listing);
    }
    
    [HttpGet("{saleChannelName}/verifyPosts")]
    public ActionResult<IEnumerable<CrossListingResult>> GetListingsByLastUpdated(string saleChannelName)
    {
